...
DEFAULT_OUTPUT_DIR = DEFAULT_OUTPUT_ROOT / "concept_annotations_masked"
...
def _expand_true_path(
    ...
    for concept_id, direct_entry in list(concept_index.items()):
        if not direct_entry["is_direct"]:
            continue
        queue = list(direct_entry.get("parents", []))
        visited: set[str] = set()
        while queue:
...
def build_concept_annotation(
    ...
    if include_true_path:
        _expand_true_path(concept_index, ontology, depths)
...
def parse_args() -> argparse.Namespace:
    ...
    if stage_block:
        if stage_block.get("input_dir") in (None, ""):
            ...
            input_default = output_root / export_output_subdirs.get(filter_mode, DEFAULT_SENTENCE_EXPORT_OUTPUT_SUBDIRS["masked"])
        if stage_block.get("output_dir") in (None, ""):
            ...
            output_default = output_root / concept_output_subdirs.get(filter_mode, DEFAULT_CONCEPT_OUTPUT_SUBDIRS["masked"])
