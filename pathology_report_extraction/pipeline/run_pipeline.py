...
def run_pipeline(config_path_arg: Path | None, filter_mode_override: str | None, limit_override: int | None) -> dict[str, Any]:
    ...
    preprocess_output_dir = _resolve_output_dir(
        ...
        default_subdirs=PREPROCESS_OUTPUT_SUBDIRS,
    )
    ...
    export_output_dir = _resolve_output_dir(
        ...
        output_root=output_root,
        filter_mode=filter_mode,
        default_subdirs=SENTENCE_EXPORT_OUTPUT_SUBDIRS,
    )
    ...
    concept_output_dir = _resolve_output_dir(
        ...
        output_root=output_root,
        filter_mode=filter_mode,
        default_subdirs=CONCEPT_OUTPUT_SUBDIRS,
    )
    ...
    encode_output_dir = _resolve_output_dir(
        ...
        output_root=output_root,
        filter_mode=filter_mode,
        default_subdirs=CONCH_OUTPUT_SUBDIRS,
    )
    ...
    if get_bool(preprocess_cfg, "enabled", True):
        from pathology_report_extraction.pipeline.preprocess_pathology_reports import process_all_pdfs
    ...
    if get_bool(export_cfg, "enabled", True):
        from pathology_report_extraction.pipeline.export_sentence_views import process_all_documents as export_sentence_views
    ...
    if get_bool(concept_cfg, "enabled", False):
        from pathology_report_extraction.ontology.extract_ontology_concepts import process_all_documents as extract_ontology_concepts
        ...
        summary = extract_ontology_concepts(
            ...
            true_path_max_ancestor_hops=(
                None
                if get_value(concept_cfg, "true_path_max_ancestor_hops", 2) in (None, "")
                else int(get_value(concept_cfg, "true_path_max_ancestor_hops", 2))
            ),
    ...
    if get_bool(encode_cfg, "enabled", True):
        from pathology_report_extraction.pipeline.encode_sentence_exports_conch import process_all_documents as encode_sentence_exports
    ...
    if get_bool(manifest_cfg, "enabled", False):
        from pathology_report_extraction.graphs.prepare_text_graph_manifest import prepare_manifest
