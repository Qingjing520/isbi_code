...
DEFAULT_OUTPUT_DIR = DEFAULT_OUTPUT_ROOT / CONCH_OUTPUT_SUBDIRS["masked"]
...
def parse_args() -> argparse.Namespace:
    ...
    if stage_block:
        if stage_block.get("input_dir") in (None, ""):
            sentence_output_subdirs = dict(SENTENCE_EXPORT_OUTPUT_SUBDIRS)
            sentence_output_subdirs.update(get_value(get_stage_config(raw_config, "export_sentence_views"), "output_subdirs", {}) or {})
            input_default = output_root / sentence_output_subdirs.get(filter_mode, SENTENCE_EXPORT_OUTPUT_SUBDIRS["masked"])
        if stage_block.get("output_dir") in (None, ""):
            ...
            output_default = output_root / conch_output_subdirs.get(filter_mode, CONCH_OUTPUT_SUBDIRS["masked"])
