...
from pathology_report_extraction.common.pdf_utils import ensure_dir, write_json, write_text
from pathology_report_extraction.common.pipeline_defaults import DEFAULT_OUTPUT_ROOT, PREPROCESS_OUTPUT_SUBDIRS, SENTENCE_EXPORT_OUTPUT_SUBDIRS
...
DEFAULT_INPUT_DIR = DEFAULT_OUTPUT_ROOT / PREPROCESS_OUTPUT_SUBDIRS["masked"]
DEFAULT_OUTPUT_DIR = DEFAULT_OUTPUT_ROOT / SENTENCE_EXPORT_OUTPUT_SUBDIRS["masked"]
...
def parse_args() -> argparse.Namespace:
    ...
    if stage_block:
        if stage_block.get("input_dir") in (None, ""):
            preprocess_output_subdirs = dict(PREPROCESS_OUTPUT_SUBDIRS)
            preprocess_output_subdirs.update(get_value(preprocess_config, "output_subdirs", {}) or {})
            input_default = output_root / preprocess_output_subdirs.get(filter_mode, PREPROCESS_OUTPUT_SUBDIRS["masked"])
        if stage_block.get("output_dir") in (None, ""):
            ...
            output_default = output_root / export_output_subdirs.get(filter_mode, SENTENCE_EXPORT_OUTPUT_SUBDIRS["masked"])
