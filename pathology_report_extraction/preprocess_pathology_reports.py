...
from pdf_utils import ensure_dir, extract_case_id, extract_native_text_from_page, write_json
from text_cleaning import clean_lines, normalize_line, normalize_text, split_sentences
...
DEFAULT_OUTPUT_DIR = DEFAULT_OUTPUT_ROOT / "pathology_report_preprocessed_masked"
...
def parse_args() -> argparse.Namespace:
    ...
    if args.output_dir is None:
        ...
        if explicit_output_dir not in (None, ""):
            ...
            output_subdirs = dict(DEFAULT_PREPROCESS_OUTPUT_SUBDIRS)
            output_subdirs.update(get_value(config, "output_subdirs", {}) or {})
            output_subdir = output_subdirs.get(args.filter_mode, DEFAULT_PREPROCESS_OUTPUT_SUBDIRS[DEFAULT_FILTER_MODE])
